...
const STAGE_TYPE: Record<number, VocabQuestionType> = {
  0: "definition",
  1: "definition",
  2: "synonym",
  3: "context",
...
export function pickNextQuestion(
  ...
  const due = words
    ...
    .sort(
      (a, b) =>
        Date.parse(a.state!.nextReview || "1970-01-01") -
        Date.parse(b.state!.nextReview || "1970-01-01"),
    );
  const unseen = words.filter((word) => !mastery[word.id]);
  const fallback = [...words].sort(
    (a, b) =>
      Date.parse(mastery[a.id]?.lastSeen || "1970-01-01") -
      Date.parse(mastery[b.id]?.lastSeen || "1970-01-01"),
  );
  const word = due[0]?.word ?? unseen[0] ?? fallback[0]!;
  const candidates = questionsFor(word.id, mastery[word.id]?.masteryStage ?? 0).filter(
...
export function answerWord(
  ...
  const nextStage = Math.min(
    3,
    state.masteryStage + (streak >= 2 ? 1 : 0),
  ) as WordMastery["masteryStage"];
  return {
    ...state,
