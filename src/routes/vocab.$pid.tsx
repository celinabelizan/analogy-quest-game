...
function pickNext(mastery: Record<string, WordMastery>, lastQuestionId: string | null): VocabQuestion {
...
function VocabDrill() {
  ...
  const choose = (choiceId: string) => {
    ...
    update((prev) => {
      ...
      if (correct) {
        ...
        if (question.type === "context" && after.contextScore >= 2 && !before.masteredBonusAwarded) {
          awarded += 10;
    ...
    if (correct) {
      ...
      flash(`+${awarded || TYPE_XP[question.type]} XP${awarded > TYPE_XP[question.type] ? " — word mastered!" : ""}`);
    }
  };
  ...
  const mastered = useMemo(() => Object.values(mastery).filter((entry) => entry.masteredBonusAwarded).length, [mastery]);
  const pct = Math.min(100, Math.round((done / VOCAB_DAILY_GOAL) * 100));

  return <main className="mx-auto min-h-dvh w-full max-w-2xl px-5 pb-24 pt-6">
    <Confetti fire={burst} />
    <AnimatePresence>{toast && <motion.div initial={{ y: -40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -40, opacity: 0 }} className="fixed left-1/2 top-4 z-50 -translate-x-1/2 rounded-full bg-primary px-5 py-2 font-bold text-primary-foreground shadow-lg">{toast}</motion.div>}</AnimatePresence>
    <header className="mb-4 flex items-center justify-between gap-3">
      <Link to="/dashboard/$pid" params={{ pid: id }} className="min-h-[44px] rounded-full border border-border px-4 py-2 text-sm">← {meta.name}'s dashboard</Link>
      <span className="rounded-full bg-primary/10 px-3 py-1 text-sm font-bold text-primary">{mastered} mastered</span>
    </header>
    <section className="quest-card mb-5 space-y-2 p-4">
      <div className="flex items-center justify-between text-sm font-bold"><span>Today's words: {done} / {VOCAB_DAILY_GOAL}</span><span>{stageLabel}</span></div>
      <div className="h-3 overflow-hidden rounded-full bg-secondary/40"><motion.div animate={{ width: `${pct}%` }} className="h-full rounded-full bg-primary" /></div>
    </section>
    <section className="quest-card space-y-4 p-7">
      <div className="flex items-start justify-between gap-3"><h1 className="font-display text-4xl font-extrabold tracking-tight">{word.word.toUpperCase()}</h1><span className="rounded-full bg-secondary px-3 py-1 text-xs font-bold uppercase">{question.type}</span></div>
      {word.root && <p className="text-base text-muted-foreground">🔑 Root: <b>{word.root}</b> = {word.rootMeaning}</p>}
      ...
      <div className="space-y-2">{choices.map((choice, index) => {
        ...
        return <div key={choice.id}><BouncyTap onClick={() => choose(choice.id)} className={`w-full border px-4 py-3 text-left text-base ${!answered ? "border-border" : isCorrect ? "border-green-600 bg-green-600/10" : isPicked ? "border-destructive bg-destructive/10" : "border-border opacity-50"}`}><b>({String.fromCharCode(65 + index)})</b> {choice.text}</BouncyTap>{show && <p className={`mt-1 px-2 text-sm ${isCorrect ? "text-green-700" : "text-destructive"}`}>{choice.why}</p>}</div>;
      })}</div>
      {answered && <BouncyTap onClick={nextWord} className="w-full bg-primary px-6 py-4 text-lg font-extrabold text-primary-foreground">Next word →</BouncyTap>}
      {answered && !right && <p className="text-center text-sm text-muted-foreground">This word will return soon. Read the explanation, then beat it next time.</p>}
    </section>
  </main>;
}
