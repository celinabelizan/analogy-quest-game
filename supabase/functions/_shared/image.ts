...
function webpSize(bytes: Uint8Array) {
  if (bytes.length < 30 || new TextDecoder().decode(bytes.slice(8, 12)) !== "WEBP") throw new Error("Invalid WebP")
...
export function inspectImage(bytes: Uint8Array, claimed: string) {
  ...
  if (starts(bytes, JPEG)) { mime = "image/jpeg"; size = jpegSize(bytes) }
  else if (starts(bytes, PNG)) { mime = "image/png"; size = pngSize(bytes) }
  else if (starts(bytes, WEBP)) { mime = "image/webp"; size = webpSize(bytes) }
  else throw new Error("Unsupported or spoofed image")
  if (claimed !== mime) throw new Error("MIME type does not match file bytes")
  if (size.width < 1 || size.height < 1 || size.width > 4096 || size.height > 4096) throw new Error("Image dimensions exceed 4096×4096")
